...
class AGOB:
    ...
    def find_table(self, user_request: str) -> SearchResult:
        ...
        try:
            ...
            # 1. Buscar tablas en OpenMetadata
            search_results = self._search_openmetadata(user_request)
            ...
            # 3. Verificar si hay coincidencia exacta
            ...
            # 4. Si no hay coincidencia exacta, generar SQL query
    ...
    def _search_openmetadata(self, query: str) -> List[Dict]:
        """
    ...
    def _search_openmetadata_alternative(self) -> List[Dict]:
        """
        Búsqueda alternativa: listar todas las tablas disponibles
        """
        try:
            # Intentar endpoint de tablas
            tables_url = f"{self.base_url}/api/v1/tables"
            
            print(f"[AGOB] 🔄 Intentando listar tablas desde: {tables_url}")
